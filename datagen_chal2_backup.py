...
def DataGeneratorCh2(num_samples, version, input_shape, input_shape_vis):
    ...
    images_vis = np.zeros((len(labels), input_shape_vis, input_shape_vis, 1))
    """ Loop thru indexes """
        ...
        for ich, ch in enumerate(channel_vis):
            ...
            if os.path.isfile(image_file_vis):
                ...
                if images_vis is None:
                    images_vis = np.zeros((nsamples, *image_data_vis.shape, len(channel_vis)))
    ...
    apply_log = True
    print('\n -- Normalizing...')
    images = utils.normalize(images, len(channels), apply_log=apply_log, vis=False)
    print('\n -- Normalizing VIS...')
    images_vis = utils.normalize(images_vis, len(channel_vis), apply_log=apply_log, vis=True)
    print(apply_log)
    ...
    #index = utils.save_clue(inputs_vis, outputs, num_samples, version, 'vis', input_shape, 5, 5, index, channels)
