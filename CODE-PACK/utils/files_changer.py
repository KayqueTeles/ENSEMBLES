...
def MOVE_TO_TRASHCAN(k_folds, version, input_shape, num_epochs, challenge_size):
    counter, weicounter, csvcounter = (0 for i in range(3))
    foldtimer = time.perf_counter()
    print('\n ** Moving specified files and folders to TRASH_CAN...')
    print(' ** Checking .png, .h5 and csv files...')
    print(" ** Checking if there's a results folder...")
    if os.path.exists('./TRASH_CAN'):
        print(" ** Trash file found. Don't forget to clean it!")
    else:
        print(' ** None found. Creating one.')
        os.mkdir('./TRASH_CAN')
        print(' ** Done!')
    dest1 = ('/home/kayque/LENSLOAD/TRASH_CAN/')
    models_list = ['resnet50', 'resnet101', 'resnet152', 'ensemble', 'ensembles',
                   'effnet_B0', 'effnet_B1', 'effnet_B2', 'effnet_B3',
                   'effnet_B4', 'effnet_B5', 'effnet_B6', 'effnet_B7',
                   'inceptionV2', 'inceptionV3', 'xception']
    challist = np.linspace(0, challenge_size, (challenge_size/10))
            ...
            for train_size in challist:
                for fold in range(int(k_folds)+1):
                    if os.path.exists('/TrainTest_rate_{}_TR_{}_version_{}.png'. format(mod, train_size, version)):
                        ...
                        counter = counter + 1
                    if os.path.exists('/TrainVal_rate_{}_TR_{}_Fold_{}_version_{}.png'. format(mod, train_size, fold, version)):
                        shutil.move('/TrainVal_rate_{}_TR_{}_Fold_{}_version_{}.png'. format(mod, train_size, fold, version))
                        counter = counter + 1
                    if os.path.exists('/AccxEpoch_{}_{}_Fold_{}_version_{}.png'. format(mod, train_size, fold, version)):
                        shutil.move('/AccxEpoch_{}_{}_Fold_{}_version_{}.png'. format(mod, train_size, fold, version))
                        counter = counter + 1
                    if os.path.exists('/LossxEpoch_{}_{}_Fold_{}_version_{}.png'. format(mod, train_size, fold, version)):
                    ...
                    if os.path.exists('/FScoreGraph_{}_Full_{}_version_{}.png'. format(mod, train_size, version)):
                        ...
                        counter = counter + 1
                    if os.path.exists('/code_data_version_{}_model_{}_aucs.csv'. format(version, mod)):
                        shutil.move('/code_data_version_{}_model_{}_aucs.csv'. format(version, mod))
                        csvcounter = csvcounter + 1
                    if os.path.exists('/code_data_version_{}.csv'. format(version)):
                        shutil.move('/code_data_version_{}.csv'. format(version), dest1)
                        csvcounter = csvcounter + 1
                    if os.path.exists('/code_parameters_version_{}.csv'. format(version)):
                        shutil.move('/code_parameters_version_{}.csv'. format(version), dest1)
                        csvcounter = csvcounter + 1
                    for jk in range(100*int(k_folds)):
                        if os.path.exists('/CLUE_FROM_DATASET_{}_samples_{}_version_{}_step_{}x{}_size_{}_num.png'. format(challenge_size, version, 'generator', input_shape, input_shape, jk)):
                            shutil.move('/CLUE_FROM_DATASET_{}_samples_{}_version_{}_step_{}x{}_size_{}_num.png'. format(challenge_size, version, 'generator', input_shape, input_shape, jk), dest1)
                            counter = counter + 1 
                        if os.path.exists('/CLUE_FROM_DATASET_{}_samples_{}_version_{}_step_{}x{}_size_{}_num.png'. format(challenge_size, version, 'generator', 66, 66, jk)):
                            shutil.move('/CLUE_FROM_DATASET_{}_samples_{}_version_{}_step_{}x{}_size_{}_num.png'. format(challenge_size, version, 'generator', 66, 66, jk), dest1)
                            counter = counter + 1 
                    for epoch in range(int(num_epochs)):
                        epo = epoch + 1
                        if epoch < 10:
...
def filemover(train_size, version, k_folds, model_list, dataset_size, input_shape, input_shape_vis):
    ...
    weicounter = 0
    channels = ['H', 'J', 'Y', 'VIS']
    args = ['generator', 'hjy', 'vis']
    inputs = [input_shape, input_shape_vis]
    models_list = np.append(model_list, ['ensembles', 'ensemble'])
    print(models_list)
    ...
    for mod in models_list:
        ...
        for fold in range(int(k_folds)+1):
            ...
            if os.path.exists('./FScoreGraph_{}_{}_Fold_{}_version_{}.png'. format(mod, train_size, fold, version)):
                ...
                counter = counter + 1
    for z in channels:
        if os.path.exists('./HIST_for_band_{}_before_norm.png.png'. format(z)):
            shutil.move('./HIST_for_band_{}_before_norm.png.png'. format(z), dest1)                    
            counter = counter + 1
        if os.path.exists('./HIST_for_band_{}_after_norm.png.png'. format(z)):
            shutil.move('./HIST_for_band_{}_after_norm.png.png'. format(z), dest1)
            counter = counter + 1
    for arg in args:
        for inp in inputs:
            if os.path.exists('./CLUE_FROM_DATASET_{}_samples_{}_version_{}_step_{}x{}_size_{}_num.png'. format(dataset_size, version, arg, inp, inp, int(g))):
                shutil.move('./CLUE_FROM_DATASET_{}_samples_{}_version_{}_step_{}x{}_size_{}_num.png'. format(dataset_size, version, arg, inp, inp, int(g)), dest3)
                counter = counter + 1
...
def last_mover(version, model_list, dataset_size, num_epochs, input_shape, input_shape_vis):
