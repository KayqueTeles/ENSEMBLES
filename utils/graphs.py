...
def distribution_graph_complete(test_count, trainval_count, train_size, name_file_rede, title_graph_rede, version):
    ...
    print('TrainTest_rate_{}_TR_{}_version_{}.png saved'.format(name_file_rede, train_size, version))
...
def distribution_graph_fold(train_count, val_count, train_size, fold, name_file_rede, title_graph_rede, version):
    ...
    print('TrainVal_rate_{}_TR_{}_Fold_{}_version_{}.png'.format(name_file_rede, train_size, fold, version))
...
def accurary_graph(history, num_epochs, train_size, fold, name_file_rede, title_graph_rede, version):
    ...
    print('AccxEpoch_{}_{}_Fold_{}_version_{}.png'.format(name_file_rede, train_size, fold, version))
...
def loss_graph(history, num_epochs, train_size, fold, name_file_rede, title_graph_rede, version):
    ...
    print('LossxEpoch_{}_{}_Fold_{}_version_{}.png'.format(name_file_rede, train_size, fold, version))

def roc_graph(rede, model, x_test, x_test_vis, y_test, title_graph_rede, train_size, fold, version, name_file_rede):
    tpr, fpr, tpr_vis, fpr_vis, auc, auc_vis, thres = utils.roc_curve_calculate(y_test, x_test, x_test_vis, model, rede)
    ...
    plt.plot(fpr_vis, tpr_vis, label='fold' + str(fold) + '& AUC: %.3f' % auc_vis, color='C' + str(fold), linewidth=3)  # for color 'C'+str(fold), for fold[0 9]
    ...
    print('ROCLensDetectNet_{}_{}_Fold_{}_version_{}.png'. format(name_file_rede, train_size, fold, version))
    return tpr, fpr, auc, tpr_vis, fpr_vis, auc_vis, thres
...
def roc_graph_ensemble(rede, model_resnet, model_effnet, x_test, y_test, title_graph_rede, train_size, fold, version, name_file_rede):
    tpr, fpr, auc, thres = utils.roc_curve_calculate_ensemble(y_test, x_test, model_resnet, model_effnet, rede, version)
    print(' ** Plotting %s ROC Graph' % rede)
    plt.figure()
    plt.plot([0, 1], [0, 1], 'k--')  # k = color black
    plt.plot(fpr, tpr, label='fold' + str(fold) + '& AUC: %.3f' % auc, color='C' + str(fold), linewidth=3)  # for color 'C'+str(fold), for fold[0 9]
...
def roc_graphs_sec(rede, models, x_test, x_test_vis, y_test, model_list, train_size, fold, version, name_file_rede):
    tpr, fpr, tpr_vis, fpr_vis, auc, auc_vis, thres = utils.roc_curves_sec(y_test, x_test, x_test_vis, models, model_list, version)
    print(' ** Plotting %s ROC Graph' % rede)
    plt.figure()
    plt.plot([0, 1], [0, 1], 'k--')  # k = color black
    plt.plot(fpr, tpr, label='fold' + str(fold) + '& AUC: %.3f' % auc, color='C' + str(fold), linewidth=3)  # for color 'C'+str(fold), for fold[0 9]
    plt.plot(fpr_vis, tpr_vis, label='fold' + str(fold) + '& AUC_VIS: %.3f' % auc_vis, color='C' + str(fold), linewidth=3)  # for color 'C'+str(fold), for fold[0 9]
    plt.legend(loc='lower right', ncol=1, mode='expand')
...
def final_roc_graph(k_folds, train_size, medians_x, medians_y, mauc, lowlim, highlim, med_y_v, med_x_v, mauc_v, low_v, hig_v, name_file_rede, title_graph_rede, version):
    plt.figure()
    plt.plot([0, 1], [0, 1], 'k--')  # k = color black
    ...
    plt.plot(med_x_v, med_y_v, 'g', label='AUC_VIS: {}'.format(mauc), linewidth=3)
    plt.fill_between(med_x_v, med_y_v, low_v, color='green', alpha=0.3, interpolate=True)
    plt.fill_between(med_x_v, hig_v, med_y_v, color='green', alpha=0.3, interpolate=True)
    plt.legend(loc='lower right', ncol=1, mode='expand')
    plt.savefig('ROCLensDetectNet_{}_Full_{}_version_{}.png'.format(name_file_rede, train_size, version))
    print('ROCLensDetectNet_{}_Full_{}_version_{}.png'.format(name_file_rede, train_size, version))

def fscore_graph(rede, model, x_test, y_test, title_graph_rede, train_size, fold, version, name_file_rede, batch_size):
    prec, rec, f_1_score, f_100_score, thres = utils.FScore_curves(rede, model, x_test, y_test, batch_size)  #(y_test, x_test, model_resnet, 'resnet')
    ...
    print('FScoreGraph_{}_{}_Fold_{}_version_{}.png'. format(name_file_rede, train_size, fold, version))
    return prec, rec, f_1_score, f_100_score

def fscore_graph_ensemble(model_list, models, x_test, y_test, train_size, fold, version, batch_size):
    prec, rec, f_1_score, f_100_score, thres = utils.FScore_curves_ensemble(y_test, x_test, models, model_list, batch_size)  #(y_test, x_test, model_resnet, 'resnet')
    print(' ** Plotting F Scores Graph')
    plt.figure()
    plt.plot([0, 1], [0.5, 0.5], 'k--')  
    plt.plot(rec, prec, label='fold' + str(fold) + '& F1: %.3f & F100: %.3f' % (f_1_score, f_100_score), color='C' + str(fold), linewidth=3)  
    ...
    print('FScoreGraph_{}_{}_Fold_{}_version_{}.png'. format('ensemble', train_size, fold, version))
    return prec, rec, f_1_score, f_100_score
...
def ultimate_ROC(lauc, thres, tpr_all, fpr_all, lauc_vis, tpr_all_vis, fpr_all_vis, name_file_rede, title_graph_rede, k_folds, train_size, rede, version):
    ...
    final_roc_graph(k_folds, train_size, medians_x, medians_y, mauc, lowlim, highlim, med_y_v, med_x_v, mauc_v, low_v, hig_v, name_file_rede, title_graph_rede, version)
...
def final_fscore_graph(k_folds, train_size, medians_x, medians_y, mauc, lowlim, highlim, name_file_rede, title_graph_rede, version):
    ...
    print('FScoreGraph_{}_Full_{}_version_{}.png'.format(name_file_rede, train_size, version))

def histo(X, vmin, vmax, before_norm, band):
    plt.hist(X.ravel(), bins = np.linspace(vmin, vmax, 100))
    plt.xlim(vmin, vmax)
    if before_norm:
        plt.title('Pixel Histogram for %s band before_norm.' % (band))
        plt.savefig('HIST_for_band_%s_before_norm.png' % band)
    else:
        plt.title('Pixel Histogram for %s band after_norm.' % (band))
        plt.savefig('HIST_for_band_%s_after_norm.png' % band)